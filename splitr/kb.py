...
class POGKeyboard(KMKKeyboard):
    def __init__(self, features=["basic"]):
        ...
        if "serial" in features:
            from pog_serial import pogSerial

            self.modules.append(pogSerial())
        ...
        if pog.config["split"]:
            ...
            if pog.keyboardType == "splitBLE":
                ...
                self.split = Split(split_type=SplitType.BLE, split_side=side)
            elif pog.keyboardType == "splitSerial":
                print("split with 2 pins (UART)")
                self.split = Split(split_side=side, split_type=SplitType.UART, split_flip=False, data_pin=pog.splitPinA, data_pin2=pog.splitPinB, use_pio=True)
            else:
        ...
        if pog.directWiring:
            self.matrix = KeysScanner(pins=eval(pog.pins), value_when_pressed=False, pull=True, interval=0.02, max_events=64)
