...
def maybe_make_shifted_key(candidate: str) -> Optional[Key]:
    ...
    for code, names in codes:
        if candidate in names:
            return make_key(names=names, constructor=ModifiedKey, code=code, modifier=KC.LSFT)
...
class ModifierKey(_DefaultKey):
    def __call__(self, key: Key) -> Key:
        ...
        if isinstance(key, ModifiedKey) and key.modifier.code & self.code == key.modifier.code:
            return key
...
class ModifiedKey(Key):
    ...
    def __repr__(self):
        return super().__repr__() + "(key=" + str(self.key) + ", modifier=" + str(self.modifier) + ")"
