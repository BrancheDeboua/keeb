        ...
        if self.split_type == SplitType.UART and self.data_pin is not None:
            if self._is_target or not self.uart_flip:
                if self._use_pio:
                    ...
                    self._uart = busio.UART(tx=self.data_pin2, rx=self.data_pin, timeout=self._uart_interval)
            else:
                if self._use_pio:
                    self._uart = self.PIO_UART(tx=self.data_pin, rx=self.data_pin2)
                else:
                    self._uart = busio.UART(tx=self.data_pin, rx=self.data_pin2, timeout=self._uart_interval)
    ...
    def _initiator_scan(self):
        ...
        if not self._uart:
            if self._debug_enabled:
                print("Scanning")
            self._ble.stop_scan()
            for adv in self._ble.start_scan(self.ProvideServicesAdvertisement, timeout=20):
                if self._debug_enabled:
                    print("Scanning")
