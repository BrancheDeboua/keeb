...
KeyBufferFrame = namedtuple("KeyBufferFrame", ("key", "is_pressed", "int_coord", "index"))


def debug_error(module, message: str, error: Exception):
    if debug.enabled:
        debug(message, ": ", error.__class__.__name__, ": ", error, name=module.__module__)
...
class KMKKeyboard:
    ...
    def process_key(self, key: Key, is_pressed: bool, int_coord: Optional[int] = None) -> None:
        if is_pressed:
    ...
    def resume_process_key(
        ...
        ksf = KeyBufferFrame(key=key, is_pressed=is_pressed, int_coord=int_coord, index=index)
